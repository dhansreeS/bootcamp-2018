...
generation_dt[,.(more_hydro = small_hydro + large_hydro)]
...
# run answer
generation_dt[,.(SW = sum(solar+wind)), by=.(days=day(datetime),hours=hour(datetime))]
generation_dt[,.(SW = sum(solar+wind)), by=.(days=day(datetime))]
generation_dt[,.(SW = sum(solar+wind)), by=.(hours=hour(datetime))]
